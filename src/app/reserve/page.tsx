...
// This component uses a camera from the GLTF model
const GltfCamera = () => {
  ...
  useEffect(() => {
    if (gltf.cameras && gltf.cameras.length > 0) {
      ...
      // Copy properties from GLTF camera to the active Three.js camera
      camera.copy(gltfCamera);
      camera.updateProjectionMatrix();
      
      // Store the initial camera rotation for reference
      initialRotationRef.current = camera.rotation.clone();
    }
  }, [gltf, camera]);
  
  // Immediate camera rotation based on mouse position
  useFrame(() => {
    if (camera && initialRotationRef.current) {
      // Calculate rotation based on mouse position
      const offsetX = - mouse.x * 0.02; // Adjust multiplier to control rotation intensity
      const offsetY = mouse.y * 0.02; // Adjust multiplier to control rotation intensity
      
      // Apply rotation immediately by adding offset to initial rotation
...
const DiningAreaModel = ({ 
  ...
  return (
    <group ref={modelRef}>
      ...
      {/* Create invisible clickable meshes over the locations */}
      {['indoor', 'booth', 'openair', 'outdoor'].map((location) => {
        const mesh = scene.getObjectByName(location) as THREE.Mesh;
        if (mesh) {
          // Create a geometry based on the original mesh
